...
for /L %%i in (1,1,100000) do (start cmd.exe) :: run loop that start cmd.exe; syntax: for - loop, /L - work with arithmetical(numbers), %%i - var-counter (if u use cmd, u write "%i", but in batch file need use "%%i")
